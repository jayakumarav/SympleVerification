...
Inputs = int32([67108864,5,67108864,4;% Int and Int
...
ExpectedOutputs = int32([67108864,1,0;
                       67108864,0,0;
                       67108864,0,0;
                       67108864,-1,0;
                       67108864,0,0;
                       16777216,0,256;
                       67108864,-715827883,256;
                       16777216,-2147483648,256;
                         ]);
...
tf = sltest.testmanager.TestFile('BXOR'); %name of the mldatx file
ts = getTestSuites(tf);
tc = getTestCases(ts);
results = run(tc(1));
...
for i = 1:TestIters
   TestInps = i;
   sim('FBFunctions_Harness_BXOR',10); % 
